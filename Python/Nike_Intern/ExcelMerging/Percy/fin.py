    ...
    if Manager_row is None: #find core manager row
        ...
        if EE_Staff_row is not None: #find staff row in EE
            ...
            if final_id is None: #not find department in Mapping sheet
                final_id=1356
                warn="New department "+str(EE.sheet.cell(row=EE_Staff_row,column=EE.function3_ID).value)+" in row "+str(row_num) #write new department warning list
                Tuser.warning_list.append(warn)
                print(warn) #not find department in mapping sheet
        else: #not find staff mail in EE sheet
            final_id=1356
            #print("warning cannot find row "+str(row_num)+" staff in EE",flush=True) #not find staff mail in EE sheet
...
if __name__ == "__main__":
    ...
    Please import 3 documens in following order:\n\n\
        ...
        +str(Mapping.add)\
            ...
            Tuser.save_excel()
            if Tuser.warning_list is not None:
                text=""
                for i in Tuser.warning_list: #list out warning content
                    text+=(str(i)+"\n")
                gui.textbox(msg="Found "+str(len(Tuser.warning_list))+" warnings:",text=text,title="Warning list")
            else:
                gui.msgbox("Finished")
            break
