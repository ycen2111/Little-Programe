...
class Excel(object):
    ...
    def find_target_row(self,column_ID,target_msg):
        ...
        for row_num in range(2,max_row):
            if str(self.sheet.cell(row=row_num,column=column_ID).value)[:-9].lower()==target_msg: #find target row number by small letter form
                return row_num
            else:
                row_num=row_num+1
...
def Userdepartment_ID_enter(Tuser,EE,Mapping):
    ...
    max_column=Tuser.sheet.max_column
...
def Find_Manager_Id(Tuser,EE,Mapping,row_num):
    ...
    if Manager_row is None: #find core manager row
        ...
        if EE_Staff_row is not None: #find staff row in EE
            ...
            for i in range(2,Mapping.sheet.max_row+1):
        ...
        if Tuser.sheet.cell(row=Manager_row,column=Tuser.sheet.max_column).value==1: #manager have been searched
    ...
    Tuser.sheet.cell(row=row_num,column=Tuser.sheet.max_column,value=1) #mark finish sign
    return final_id
    ...
    Please import 3 documens in following order:\n\n\
        ...
        +str(Mapping.add)\
            ...
            if Tuser.add=="None" or EE.add=="None" or Mapping.add=="None": #missing file
                ...
                #for row_num in range(2,Tuser.sheet.max_row+1):
                #    Tuser.sheet.cell(row=row_num,column=Tuser.userdepartmentid_ID,value="0")
