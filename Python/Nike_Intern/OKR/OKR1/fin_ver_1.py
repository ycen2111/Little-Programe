...
def analyze_msg(pdf_page,excel,msg,row):
    ...
    for line in msg_in_line:
        if column_id>11: #out of limited column list's length
            overlength=True
            print("overlength")
            excel.warning_list.append("Out of range in page "+str(row-1))
            break
        else: #remove chars coordinates shift
            while pdf_page.chars[chars_count]["text"]!=line[0]:
                chars_count-=1
        ...
        if (ord(line[0])>47 and ord(line[0])<58) and ord(line[1])==46: #found "n."
            ...
            column_id+=1
            #print("3")
        elif ord(line[0])>64 and ord(line[0])<91 and pdf_page.chars[chars_count]["non_stroking_color"] == (0.929, 0.49, 0.192): #first char in line is large letter and with colors
            if column_id!=0: #pass recording if this is this page's first obj.
