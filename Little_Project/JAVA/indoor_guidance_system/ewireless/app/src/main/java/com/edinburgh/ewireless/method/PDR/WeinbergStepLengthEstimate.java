...
/**
 * Author: yijianzheng
 * Date: 11/04/2023 17:03
 * <p>
 * Notes:
 */
public class WeinbergStepLengthEstimate{
    ...
    private static String TAG = "WeinbergStepLengthEstimate";
    ...
    boolean isStartCheckMinValue, isStartCheckMaxValue;
    ...
    public void reset(){
        ...
        accTempMax = 9.8;
        accTempMin = 9.8;
        posLat = UserPosition.getLatitude();
        posLon = UserPosition.getLongitude();
        CurrUserPosition.setCurrLatitude(posLat);
        CurrUserPosition.setCurrLongitude(posLon);
        isStartCheckMinValue = false;
        isStartCheckMaxValue = true;
        stepDetectOut.reset();
        Log.d(TAG, "reset: Initialize step detector, posLat:" + posLat + " posLon: " + posLon);
    }

    public StepDetectOut onStartCommand(float[] accValue , float angle) {
        accNowValue = accValue.clone();
...
//        boolean isStartCheckMinValue = false, isStartCheckMaxValue = true;
            ...
            if (isWalk()) {     //  The user is in motion
                ...
                if (!isStartCheckMaxValue && accSlidingWindow[15] >= 9.0) {
                    stepCount++;
                    stepDetectOut.setStepCount(stepCount);
                    stepDetectOut.setStep(true);
                    ...
                    stepDetectOut.setStep(step);
                    stepDetectOut.setTimeStamp(startTime);
                    String[] llStr = disToLL(step, posLat, posLon, angle).split(",");
                    //Log.d(TAG, "onStartCommand: String:" + Double.valueOf(llStr[0]) + " " +Double.valueOf(llStr[1]) );
                    posLat = Double.valueOf(llStr[0]);
                    posLon = Double.valueOf(llStr[1]);
                    CurrUserPosition.setCurrLatitude(posLat);
                    CurrUserPosition.setCurrLongitude(posLon);
                    CurrUserPosition.setTimeStamp(System.currentTimeMillis());
                    ...
                    stepDetectOut.setStep(false);
                }
            } else {    // The user is at rest
                posLat = CurrUserPosition.getCurrLatitude();
                posLon = CurrUserPosition.getCurrLongitude();
            }
        }

        return stepDetectOut;
    }
...
//            }
    ...
    private boolean isWalk() {
        //Log.d(TAG, "isWalk: getAccStd()" + getAccStd());
