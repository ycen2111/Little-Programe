#pack code: pyinstaller --name="OKR_System" --window --onefile --upx-dir="D:\Python\Intern\OKR_env\Scripts" --icon="D:\Python\Intern\OKR\icons.ico" --distpath="D:\Python\Intern\OKR" D:\Python\Intern\OKR\fin_ver_1.py 
...
def analyze_msg(pdf_page,excel,msg,row):
    ...
    for line in msg_in_line:
        ...
        if len(line)<4:
            temp_msg+=line
            print("type 3")
        elif (ord(line[0])>47 and ord(line[0])<58) and ord(line[1])==46 and ord(line[2])==32: #found "n. " <--(note have a space here)
            if ord(line[0])>51: #more than 3 kr in an obj
                print("kr overlength")
            ...
            temp_msg=line[3:]
            print("type 1")
        elif ord(line[0])>64 and ord(line[0])<91 and pdf_page.chars[chars_count]["non_stroking_color"] != 0: #first char in line is large letter and with colors
